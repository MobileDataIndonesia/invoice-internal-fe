...
export const toggleVoidInvoice = createAsyncThunk(
  "invoice/toggleVoidInvoice",
  async (id, { rejectWithValue }) => {
    try {
      ...
      if (action.payload.status === "") {
        ...
        const filteredData = allInvoices.filter((invoice) => {
          if (action.payload.status === "void") {
            ...
            return (
              invoice.payment_status === action.payload.status &&
    ...
    builder
      ...
      .addCase(editInvoice.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          const filteredIndex = state.filteredInvoice.findIndex(
            (invoice) => invoice.invoice_id === updatedInvoice.invoice_id
          );
          if (filteredIndex !== -1) {
